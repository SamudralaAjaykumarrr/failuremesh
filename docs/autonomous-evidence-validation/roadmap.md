...
2. **Packet wire schema and legacy projection:** Should a versioned AEP sidecar reference current AC schema 1, or should a new AC schema carry claim references directly? The design chooses an envelope conceptually; later implementation must pin field types, required domains and migration behavior without silently changing legacy evaluators.
...
5. **Freshness and contradiction profiles:** Which predicates bind immutable builds versus moving operational state; what invalidation events and revalidation windows apply? Default is exact-version scope and UNKNOWN for an unestablished current claim. Tests must cover explicit corrections and unresolved same-scope disagreement.
